...
function verificarCartasArena(){
    ...
    if (campoPlayer.children.length === 1 && campoAdversario.children.length === 1){
        ...
        if (cartaAtivaPlayer.classList.contains("flip-card_back")){
            setTimeout(()=>{cartaAtivaPlayer.children[0].setAttribute("src", "../Carta (parte de tras)/carta_parte_de_tras.png")},300)
...
function regrasJokenpo(){
    ...
    if (cartaAdversario === cartaPlayer){
        resultadoPartida.setAttribute("src", "../sprites/TIED-MATCH.gif")
        qtdEmpates += 1
    }

    else if (cartaAdversario === "pedra" && cartaPlayer !== "papel"){
        resultadoPartida.setAttribute("src", "../sprites/YOU-LOSE.gif")
        qtdDerrotas += 1
    }
        ...
        resultadoPartida.setAttribute("src", "../sprites/YOU-WIN.gif")
        qtdVitorias += 1
    }
...
function timerIniciarPartida(){
    ...
    if (timerInteration <= 3 && timerInteration > 0){
        ...
        timer.setAttribute("src", `../sprites/timer_${timerInteration}.png`)
        ...
        timer.setAttribute("src", `../sprites/Fight.png`)
